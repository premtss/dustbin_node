...
//   });
 ...
 var dustbin = {
    ...
    dustbinfiltertypenew:function(limit,offset,wid,dataper,callback){
        ...
        if(wid!=="" && dataper==""){
         db.query('SELECT count(*) as total FROM `dustbins` INNER JOIN warehouses on warehouses.id=dustbins.warehouse_id WHERE dustbins.id not in(select did from assign_group_vehicle and drivers.status=1) and dustbins.warehouse_id=?',[wid],function(error,data) {
        if (error) throw error;                
        var total_pages = Math.ceil(parseInt(data[0].total) / parseInt(offset));


        var sqlquery1 ="SELECT  dustbins.*,warehouses.name as wname,warehouses.latitude as warelatitude,warehouses.longitude as warelongitute,warehouses.address as warehouseaddress,warehouses.id as warehouseID,(SELECT COUNT(*) as total FROM vehicles LEFT JOIN mapping_vehicle_drivers on mapping_vehicle_drivers.vehicle_id=vehicles.id LEFT JOIN warehouse_mapped_vehicles on warehouse_mapped_vehicles.vehicleid=vehicles.id LEFT JOIN drivers on drivers.id=mapping_vehicle_drivers.driver_id WHERE warehouse_mapped_vehicles.warehouse_Id=warehouseID and vehicles.available_status=0 and vehicles.status=1 and drivers.driverAblible=0 and drivers.status=1) as avabileTotal,(SELECT COUNT(*) as total FROM vehicles LEFT JOIN mapping_vehicle_drivers on mapping_vehicle_drivers.vehicle_id=vehicles.id LEFT JOIN warehouse_mapped_vehicles on warehouse_mapped_vehicles.vehicleid=vehicles.id LEFT JOIN drivers on drivers.id=mapping_vehicle_drivers.driver_id WHERE warehouse_mapped_vehicles.warehouse_Id=warehouseID and vehicles.available_status=1 and vehicles.status=1 and drivers.driverAblible=1 and drivers.status=1) as notavabileTotal FROM `dustbins` INNER JOIN warehouses on warehouses.id=dustbins.warehouse_id WHERE dustbins.id not in(select did from assign_group_vehicle and drivers.status=1) and dustbins.warehouse_id=? LIMIT "+start_index+", "+items_per_page+"";
    ...
    db.query('SELECT count(*) as total FROM `dustbins` INNER JOIN warehouses on warehouses.id=dustbins.warehouse_id WHERE dustbins.id not in(select did from assign_group_vehicle and drivers.status=1) and dustbins.data_percentage BETWEEN 0 and ?',[dataper],function(error,data) {
        if (error) throw error;                
        var total_pages = Math.ceil(parseInt(data[0].total) / parseInt(offset));
    ...
    db.query('SELECT count(*) as total FROM `dustbins` INNER JOIN warehouses on warehouses.id=dustbins.warehouse_id WHERE dustbins.id not in(select did from assign_group_vehicle and drivers.status=1)',function(error,data) {
        if (error) throw error;                
        var total_pages = Math.ceil(parseInt(data[0].total) / parseInt(offset));
